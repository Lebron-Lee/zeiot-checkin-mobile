...
## v4.14 修复（2026-02-28）
- [x] 修复大屏AI问答题库只有20道的问题（seedQuizQuestionsIfEmpty改为检查题目数量，不足时清空旧题重新插入150道新题）
- [x] 26个测试全部通过

## v4.15 需求/修复（2026-02-28）
- [x] 手机端拍照上传前压缩图片（canvas压缩到最大800px、质量0.8）
- [x] 刘雪丽签到使用默认头像（头像上传CDN，写入event_config表，后端签到时优先使用配置头像）
- [x] 大屏签到墙头像显示（头像已正确传入网格，无头像时显示姓名首字）
- [x] 大屏签到列表去重（同一userName只保留最新签到，初始化和WebSocket实时更新均去重）
- [x] 大屏签到列表时间显示（使用toLocaleTimeString本地时区）
- [x] 大屏签到列表滚动速度自适应（<5条 0.15px/ms，5-10条 0.25px/ms，>10条 0.35px/ms）
- [x] 后台心愿墙列表增加「发红包」按钮（点击即广播到大屏）
- [x] 大屏端新增RedPacketModal红包弹窗（红色渐变背景、金色边框、红包雨动画、显示收件人姓名和心愿内容、10秒自动关闭）
- [x] 26个测试全部通过

## v4.16 需求/修复（2026-02-28）
- [x] 大屏右侧底部4个卡片样式缩小（改为横排紧凑布局，内边距从 p-3 降为 py-1.5 px-2，数字字体从 2xl 降为 sm）
- [x] 大屏左侧签到墙头像卡片放大（左侧宽度从 420px 加到 480px）
- [x] 大屏左侧签到墙头像下方显示名字（头像下方增加半透明黑色底条，显示用户姓名）
- [x] 验证发红包WebSocket功能：后端路由已注册、广播逻辑正确、前端类型定义包含RED_PACKET、BigScreen处理逻辑完整
- [x] 验证刘雪丽默认头像：数据库已有avatar_override_刘雪丽配置，签到时优先使用该CDN头像
- [x] 26个测试全部通过

## v4.17 需求/修复（2026-02-28）
- [ ] 修复大屏签到墙头像方形平铺（头像照片铺满整个方形区域）
- [ ] 补充30道简单题目（大众易知）
- [ ] 大屏出题策略：每隔2题出一次简单题

## v4.18 修复（2026-02-28）
- [x] 修复大屏签到墙头像实时更新：WebSocket收到NEW_CHECKIN时，id存在则替换（而非跳过），确保重新签到后头像立即更新到网格
- [x] 26个测试全部通过
